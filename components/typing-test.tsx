...
interface TypingTestProps {
  ...
  onTestComplete?: (stats: ResultStats) => void
...
export function TypingTest(props: TypingTestProps) {
  ...
  const reportedResultRef = useRef<ResultStats | null>(null)

  useEffect(() => {
    if (!showResults || !frozenStats) {
      reportedResultRef.current = null
      return
    }
    if (reportedResultRef.current === frozenStats) return

    reportedResultRef.current = frozenStats
    props.onTestComplete?.(frozenStats)
  }, [showResults, frozenStats, props])
